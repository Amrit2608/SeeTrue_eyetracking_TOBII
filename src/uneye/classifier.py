...
class DNN():
    ...
    def train(self, X, Y, Labels, seed=1):
        ...
        # fixed seed to control random data shuffling in each epoch
        ...
        if any((xdim != 2, ydim != 2, ldim != 2)):
            ...
            Labels = np.reshape(
                Labels[:n_trials*trial_len], (n_trials, trial_len))
        ...
        if n_time % (self.mp**2) != 0:
            ...
            Labels = Labels[:, :int(
                np.floor(n_time/(self.mp**2))*(self.mp**2)), :]

        # validation and training set
        # 50 samples of training data used for validation
        # fixed number of validation samples independent of number of training samples
        n_validation = self.val_samples
        ...
        # torch Variables:
        Vtrain = Variable(torch.FloatTensor(
            Vtrain).unsqueeze(1), requires_grad=False)
        Ltrain = np.swapaxes(Ltrain, 1, 2)
        Ltrain = Variable(torch.FloatTensor(
            Ltrain.astype(float)), requires_grad=False)
        Vval = Variable(torch.FloatTensor(
            Vval).unsqueeze(1), requires_grad=False)
        Lval = np.swapaxes(Lval, 1, 2)
        Lval = Variable(torch.FloatTensor(
            Lval.astype(float)), requires_grad=False)
        ...
        while epoch <= self.max_iter:
            ...
            for niter in range(iters):
                ...
                # loss storage in each iteration
            ...
            if len(Loss_val) > 3:
                # validation performance better than average over last 3
                if Loss_val[-1] < float(np.mean(Loss_val[-4:-1])):
                    getting_worse = 0
                    if Loss_val[-1] < best_loss:
                        ...
                        # load weights of last time when loss was lower
                        self.net.load_state_dict(uneye_weights)

                else:  # validation performance worse than last
                    # learning rate decay:
                    # reduce learning rate by a fixed step
    ...
    def predict(self, X, Y):
        ...
        w = torch.load(self.weights_name)
    ...
    def test(self, X, Y, Labels):
        ...
        if classes == 2:
            ...
            pred = Pred == 1
            Kappa = cohenskappa((Labels == 1).astype(
                float).flatten(), pred.astype(float).flatten())
            ...
            for c in range(classes):
                pred = Pred == c
                kappa = cohenskappa((Labels == c).astype(
                    float).flatten(), pred.astype(float).flatten())
                Kappa[c] = kappa
    ...
    def crossvalidate(self, X, Y, Labels, X_val, Y_val, Labels_val, Labels_test=None, K=10):
        ...
        if any((xdim != 2, ydim != 2, ldim != 2)):
            ...
            n_trials = int(time_points/trial_len)
            X_val = np.reshape(
                X_val[:n_trials*trial_len], (n_trials, trial_len))
            Y_val = np.reshape(
                Y_val[:n_trials*trial_len], (n_trials, trial_len))
            Labels_val = np.reshape(
                Labels_val[:n_trials*trial_len], (n_trials, trial_len))
        ...
        for i in range(K):
            ...
            # torch Variable:
            Vtrain = Variable(torch.FloatTensor(
                V).unsqueeze(1), requires_grad=False)
            Ltrain = Variable(torch.FloatTensor(
                Ltrain.astype(float)), requires_grad=False)
            ...
            V = np.swapaxes(np.swapaxes(V, 0, 1), 1, 2)
            # torch Variable:
            Vtest = Variable(torch.FloatTensor(
                V).unsqueeze(1), requires_grad=False)
            ...
            while epoch < self.max_iter:
                ...
                Vtrain = Vtrain[rand_ind, :]
                Ltrain = Ltrain[rand_ind, :]
                # preallocate vector for loss storage
                loss_train = np.zeros(iters)
                for niter in range(iters):
                    # Minibatches:
                    if niter != iters-1:
                ...
                # append average loss over all iterations
            ...
            if self.classes == 2:
                ...
                # predict class that maximizes the softmax
                Prediction = np.argmax(out_test.data.cpu().numpy(), 1)
            for c in range(classes):
                pred = Prediction == c
                kappa = cohenskappa(
                    Ltest[:, c, :].flatten(), pred.astype(float).flatten())
                Kappa[i, c] = kappa
                print(kappa)
            ...
            torch.save(uneye_weights, os.path.join(
                out_folder, 'crossvalidation_'+str(i)))
